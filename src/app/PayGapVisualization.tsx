...
export function PayGapVisualization() {
  const location = useLocation();
  const state = location.state as any;

  const currentSalary = state?.currentSalary || 6000;
  const averageSalary = state?.averageSalary || 7800;
  const gapPercentage = state?.gapPercentage || 23;
  ...
  const data = [
    {
      ...
      name: "Tu Salario",
      value: currentSalary,
      fill: "#FF6B6B" // accent (Confidence Coral)
    },
    {
      id: "average",
      name: "Promedio del Mercado",
      value: averageSalary,
      fill: "#4B1D3F" // primary (Deep Plum)
    }
  ];
      ...
      <div className="w-full grid md:grid-cols-5 gap-8 lg:gap-12 items-center">
        ...
        >
          ...
          <div className="bg-primary text-primary-foreground rounded-3xl p-8 shadow-xl relative overflow-hidden">
            ...
            <h3 className="text-4xl font-bold font-heading text-secondary mb-2">
              {gapPercentage}%
            </h3>
